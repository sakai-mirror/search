	...
	public Iterator getSiteContentIterator(final String context)
	{
		...
		return new Iterator()
		{
				...
				while (ci.hasNext())
				{
					...
					try
					{
						mc = messageService.getChannel(messageService
								.channelReference(context, chanellId));
						// TODO: Fix to getCount() for trunk
						messageCount = mc.countMessages();
						if (messageCount > 0 )
						{
			...
			public Object next()
			{
				...
				try 
				{
					// TODO: In trunk - change this to getMessages(Filter ...)
					messages = mc.getMessagesSearch(null, true, pages);
